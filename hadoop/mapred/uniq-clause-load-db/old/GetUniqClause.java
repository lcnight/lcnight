...
public class GetUniqClause extends Configured implements Tool {
 ...
 {
     if (args.length != 2) {
         System.err.println("Usage: GetUniqClause <input-dir> <out-dir>");
         System.exit(-1);
     }

     int res = ToolRunner.run(new Configuration(), new GetUniqClause(), args);

     System.out.printf("run to end with return code: %d\n", res);
