...
$result = $db->_fetch("select distinct channelid from t_game_info where gameid = $gameid and paid = -1 and regionid = -1  and time >= $timestart order by channelid", 'all');
...
foreach($channelids as $id) {
    for($i = 3; $i < $idxMax; ++$i) {
        ...
        $value = $db->_fetch("select $type from t_game_info where gameid = $gameid and paid = -1 and regionid = -1  and time = $time and channelid = $id", 'cell');
        if($value == null) {
            $value = 0;
        } else {
            $value = intval($value);
