...
echo `echo $1 | grep -o '[[:digit:]]\+' | head -n 1`
}
...
get_quarter() { # get quarter curr month/day belonged to
    # month(YYYYMM) or day(YYYYMMDD)
    if [[ $1 == '' || $1 =~ [^0-9] ]]; then return;fi
    local pmonth=$1
    if [[ ${#pmonth} -gt 7 ]]; then pmonth=${pmonth:0:6}; fi
    local montmp=${pmonth:4:2}
    local yeartmp=${pmonth:0:4}

    #let "resquarter=($montmp-($montmp-1)%3)"
    case $montmp in
        01)resquarter=01;;
        02)resquarter=01;;
        03)resquarter=01;;
    ...
    echo "$yeartmp$resquarter"
}
#get_quarter 20120101
#get_quarter 20120918
#get_quarter 201209300
