...
int log_init(const char* dir, log_lvl_t lvl, uint32_t size, int maxfiles, const char* pre_name)
{
	 ...
	 */
	if ( size > (uint32_t)(1 << 31) ) {  // my add: clear up warning comparison between signed and unsigned integer expressions
	...
	if (log_buffer == MAP_FAILED) {
		log_buffer = (char*)mmap(0, log_buf_sz, PROT_WRITE | PROT_READ, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
		if (log_buffer == MAP_FAILED) {
			fprintf(stderr, "mmap log buffer failed, %m\n");
			goto exit_entry;
		}
