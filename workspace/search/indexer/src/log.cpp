...
void write_log(int lvl,uint32_t key, const char *fmt, ...)
{
	...
	//int end = vsnprintf(log_buffer + pos, log_buf_sz - pos - 1, fmt, ap);
    //log_buffer[log_buf_sz - 1] = 0;

    //big log content support
    const static int big_log_sz = 1024 * 1024;
    static char log_buffer[big_log_sz] = {0};
    int pos = snprintf(log_buffer, big_log_sz, "[%02d:%02d:%02d] %u [%05d]", tm.tm_hour, tm.tm_min, tm.tm_sec, key, getpid());
    int end = vsnprintf(log_buffer + pos, big_log_sz - pos - 1, fmt, ap);
	va_end(ap);
